...
export class MediaWDataComponent implements OnInit {
  ...
  joinChat() {
    ...
    if (this.roomId && this.roomId_list.includes(this.roomId)) {
      this.$openButton.disabled = true;
      ...
      this.callee_database.on('child_added', this.addCallerSdpAndIce.bind(this));

      this.doCreateDataChannel();
    } else {
      console.error('Can not find this room.');
   ...
   * 4. Set event handler.
  */
  addCallerSdpAndIce(data) {
    if (data.val().sender !== this.myId) {
      if (data.val().sdp) {
        const sdpMsg = JSON.parse(data.val().sdp);
        if (sdpMsg.type === 'offer' && sdpMsg.sdp) {
          console.log(`add caller sdp via setRemoteDescription`);
          this.localConnection.setRemoteDescription(new RTCSessionDescription(sdpMsg))
